...
function calcularDesconto(salario, percentual) {
  return (salario * percentual) / 100;
}

function calcularImpostoComBaseNoSalario(salario) {
...
function valorAtransferir() {
  return (
    salario -
    calcularDesconto(salario, calcularImpostoComBaseNoSalario(salario)) +
    beneficios
  );
}

function main() {
  const salarioComDesconto = valorAtransferir();
  print(
    `O salário a ser transferido para um funcionário é de R$: ${salarioComDesconto}`
  );
}
